 ...
 * 23/1/15
 * 	- break out into a plugin for no-openslide
 */
...
/* The vips_openslideload() C wrapper is provided by core vips, for
 * compatibility.
 */
...
{
#ifdef DEBUG
	printf( "vips_openslideload: module init\n" ); 
#endif /*DEBUG*/

	/* Only add us if this vips is missing openslideload.
	 */
	if( !vips_type_find( "VipsOperation", "openslideload" ) ) {
#ifdef DEBUG
		printf( "vips_openslideload: adding openslideload\n" ); 
#endif /*DEBUG*/
